# L02b: WritingFunctions

load(url("http://www.math.usu.edu/~symanzik/teaching/2013_stat6910_004_fall/Files/rainfallCO.rda"))
...
# Note that a function has at most one return value - but that can 
# be **anything**

# Return value for a linear model

lmFitResult = lm.fit(x = as.matrix(1:10), y = 1:10 + rnorm(10))
class(lmFitResult)
names(lmFitResult)
lmFitResult$coefficients
lmFitResult$residuals


# Return value of colorRampPalette

library(graphics)

pal = c("red", "green")

bpal = colorRampPalette(pal)(50)
bpal2 = colorRampPalette(pal)(20)


pie(rep(1, 50), col = bpal, clockwise = TRUE)
bpal

pie(rep(1, 20), col = bpal2, clockwise = TRUE)
bpal2
...
# As a side note, what does 1L (etc.) stand for?
...
# read here for more in-depth details:
